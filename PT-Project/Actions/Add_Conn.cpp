...
void Add_Conn::ReadActionParameters()
{
	...
	} while (S == NULL || !valid || ((dynamic_cast<Start_End*>(S) && ((Start_End*)S)->getKind() == 0))); // 0 for End

	do {
		o->PrintMessage("Click on The valid Destination Statemnt : ");
		Point p;
		i->GetPointClicked(p);
...
void Add_Conn::Execute()
{
	...
	if (pManager->Statementcurrent() >= 2) {
		for (int i = 0; i < pManager->Statementcurrent(); i++) {
			if ((dynamic_cast<Start_End*>(pManager->Statementlist()[i]) && ((Start_End*)pManager->Statementlist()[i])->getKind() == 0)) { //0 for end
				continue;
			}
			Connector **arr; int n;
